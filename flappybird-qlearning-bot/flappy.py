...
def main():
	...
	while True:
		...
		# save screenshot of maxscore
		if crashInfo['score'] >= bot.max_score:
...
def pixelCollision(rect1, rect2, hitmask1, hitmask2):
	...
	for x in range(rect.width):
		for y in range(rect.height):
			if x1 + x < len(hitmask1) and y1 + y < len(hitmask1[x1+x]) and hitmask1[x1+x][y1+y] \
					and x2 + x < len(hitmask2) and y2 + y < len(hitmask2[x2+x]) and hitmask2[x2+x][y2+y]:
				return True
	return False
