...
class Bot(object):
	...
	def __init__(self):
		...
		self.DUMPING_N = 100  # Number of iterations to dump Q values to JSON after
		self.discount = 1.0
	...
	def map_state(xdif, ydif, vel):
		...
		# The state is a string, "xdif_ydif_nextydif_vel"

		# X -> [-40,-30...120] U [140, 210 ... 420]
		# Y -> [-300, -240, -180] U [-170, -160... 160] U [180, 240 ... 420]
		if xdif < 140:  # TODO: more precious for around the border?
	...
	def dump_qvalues(self):
		...
		if self.gameCNT % self.DUMPING_N == 0:
			with open('qvalues.json', 'w') as fil:
			...
			self.last_scores = []
			print('Iteration {}. Q-values updated on local file.'.format(self.gameCNT))
